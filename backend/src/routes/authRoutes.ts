...
router.post("/register", async (req: Request, res: Response) => {
	...
	if (error) return res.status(400).send({ message: "Validation failed", error: error.details[0].message });
...
router.post("/reset-password/:id/:token", (req, res) => {
	...
	if (!secret) return res.status(401).send({ message: "Invalid or expired token" });
	else {
		jwt.verify(token, secret, async (err, decoded) => {
			if (err) {
				...
				try {
					...
					return res.status(500).send({ message: "An error occurred while saving the user", error: e });
				}
			}
		});
	}
